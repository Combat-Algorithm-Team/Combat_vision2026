...
// Armor size, Unit: m
constexpr double SMALL_ARMOR_WIDTH = 130 / 1000.0; // 135
constexpr double SMALL_ARMOR_HEIGHT = 50 / 1000.0; // 55
constexpr double LARGE_ARMOR_WIDTH = 225.0 / 1000.0;
constexpr double LARGE_ARMOR_HEIGHT = 50.0 / 1000.0; // 55

//Armor bound size, Unit: m
constexpr double LIGHT_WIDTH = 8.15 / 1000.0;
constexpr double INTERNAL_DEEPTH = 2.7 / 1000.0;
constexpr double EXTERNAL_DEEPTH = 3.0 / 1000.0;//not measured yet
...
struct Armor {
  ...
  static inline std::vector<PointType> buildBoundPoints(const double &w, const double &h,
                                                        const double &lw, const double &in_d,
                                                        const double &ex_d, const bool &clock) noexcept {
    //clock : 0 for clockwise, 1 for counter clockwise;
    if(clock == 0){
      return{PointType(-in_d, (w + lw) / 2, -h / 2),
             PointType(-in_d, (w + lw) / 2, h / 2),
             PointType(-ex_d, (-w + lw) / 2, h / 2),
             PointType(-ex_d, (-w + lw) / 2, -h / 2)};
    } else {
      return{PointType(-ex_d, (-w + lw) / 2, -h / 2),
             PointType(-ex_d, (-w + lw) / 2, h / 2),
             PointType(-in_d, (w + lw) / 2, h / 2),
             PointType(-in_d, (w + lw) / 2, -h / 2)};
    }
