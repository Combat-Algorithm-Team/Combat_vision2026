...
void ProtocolSentry::send(const rm_interfaces::msg::GimbalCmd &data) {
  ...
  packet_.loadData<unsigned int>(static_cast<unsigned int>(data.header.stamp.sec),16);
  packet_.loadData<unsigned int>(static_cast<unsigned int>(data.header.stamp.nanosec),20);
  // // chassis control
  // // linear x
