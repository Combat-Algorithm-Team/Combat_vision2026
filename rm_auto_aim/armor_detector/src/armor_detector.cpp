...
Detector::Detector(const int &bin_thres, const EnemyColor &color, const LightParams &l,
                   const ArmorParams &a)
...
std::vector<Armor> Detector::detect(const cv::Mat &input) noexcept
{
    ...
    if (!armors_.empty() && classifier != nullptr) {
        // Parallel processing
        std::for_each(std::execution::par, armors_.begin(), armors_.end(),
...
std::vector<Light> Detector::findLights(const cv::Mat &rgb_img, const cv::Mat &binary_img) noexcept
{
    using std::vector;
...
ArmorType Detector::isArmor(const Light &light_1, const Light &light_2) noexcept
{
    ...
    // if (! light_ratio_ok){
    //   std::cout<<"light ratio not ok: "<<light_length_ratio<<std::endl;

    // }
    // if (! center_distance_ok){
    //   std::cout<<"center distance not ok: "<<center_distance<<std::endl;

    // }
    // if (! angle_ok){
    //   std::cout<<"angle not ok: "<<angle<<std::endl;}
