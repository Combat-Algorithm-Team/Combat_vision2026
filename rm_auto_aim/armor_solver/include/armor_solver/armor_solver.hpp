...
public:
    ...
    // Throw: tf2::TransformException if the transform from "odom_vision" to "gimbal_link" is not
    // available
...
private:
    ...
    std::vector<Eigen::Vector3d> getArmorPositions(const Eigen::Vector3d &target_center,
                                                   const double yaw, const double r1,
    ...
    void calcYawAndPitch(const Eigen::Vector3d &p, const std::array<double, 3> rpy, double &yaw,
                         double &pitch) const noexcept;

    bool isOnTarget(const double cur_yaw, const double cur_pitch, const double target_yaw,
