                                            ...
                                            std::shared_ptr<tf2_ros::Buffer> tf2_buffer_)
{
    ...
    // std::cout<<"Target x y z yaw: "<<target_position.x()<<' '<<target_position.y()<<'
    // '<<target_position.z()<<' '<<target_yaw<<std::endl;
    ...
    int idx = selectBestArmor(armor_positions, target_position, target_yaw, target.v_yaw,
                              target.armors_num);
    ...
    switch (state) {
        case TRACKING_ARMOR: {
            ...
            // If isOnTarget() never returns true, adjust controller_delay to force the gimbal to
            // move
...
bool Solver::isOnTarget(const double cur_yaw, const double cur_pitch, const double target_yaw,
...
std::vector<Eigen::Vector3d> Solver::getArmorPositions(const Eigen::Vector3d &target_center,
                                                       ...
                                                       const double r2, const double d_zc,
                                                       const double d_za,
...
void Solver::calcYawAndPitch(const Eigen::Vector3d &p, const std::array<double, 3> rpy, double &yaw,
                             double &pitch) const noexcept
{
    // Calculate yaw and pitch
    // std::cout<<"x y yaw:"<< -p.x()<<' '<<p.y()<<' '<<atan2(p.y(), -p.x())<<std::endl;
    yaw = atan2(p.y(), p.x());
