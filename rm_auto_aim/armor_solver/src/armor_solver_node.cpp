    ...
    : Node("armor_solver", options), solver_(nullptr)
{
    ...
    target_frame_ = this->declare_parameter("target_frame", "odom_vision");
    ...
    set_mode_srv_ = this->create_service<rm_interfaces::srv::SetMode>(
        "armor_solver/set_mode", std::bind(&ArmorSolverNode::setModeCallback, this,
...
void ArmorSolverNode::armorsCallback(const rm_interfaces::msg::Armors::SharedPtr armors_msg)
{
    ...
    armors_msg->armors.erase(std::remove_if(armors_msg->armors.begin(), armors_msg->armors.end(),
    ...
    //   double freq = 100.0 / std::chrono::duration<double>(now - last_time).count();
    //   FYT_INFO("armor_solver", "armorsCallback 平均频率: {:.2f} Hz (100次)", freq);
    //   last_time = now;
    //   count = 0;
    // }
}
